jQuery(function() {
    ...
    function i() {
        var e, a = "";
        "ready" === k ? a = "选中" + m + "张图片，共" + WebUploader.formatSize(h) + "。": "confirm" === k ? (e = n.getStats(), e.uploadFailNum && (a = "已成功上传" + e.successNum + "张照片至服务器，" + e.uploadFailNum + '张照片上传失败，<a class="retry" href="#">重新上传</a>失败图片或<a class="ignore" href="#">忽略</a>')) : (e = n.getStats(), a = "共" + m + "张（" + WebUploader.formatSize(h) + "），已上传" + e.successNum + "张", e.uploadFailNum && (a += "，失败" + e.uploadFailNum + "张")),
        p.html(a)
    }
    function t(e,file, response) {
        var a;
        if (e !== k) {
    ...
    n = WebUploader.create({
        ...
        disableGlobalDnd: !0,
        chunked: !0,
        server: $("#Path").val()+"ProdPicture/addProduct",
        fileNumLimit: 5,
    ...
    n.on("all",
    function(e,file, response) {
        switch (e) {
        case "uploadAccept":
            t("uploadAccept",file, response);
            break;
        case "uploadFinished":
            t("confirm",file, response);
            break;
        case "startUpload":
            t("uploading",file, response);
            break;
        case "stopUpload":
            t("paused",file, response)
        }
    }),
