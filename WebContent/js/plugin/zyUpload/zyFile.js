...
var ZYFILE = {
		...
		funGetFiles : function(e){  
			if(ZYFILE.uploadFile.length>=3){
				webToast("最多选择三张图片上传~","bottom",3000);
				return false;
			}
			var self = this;
			// 取消鼠标经过样式
		...
		funDeleteFile : function(delFileIndex, isCb){
			...
			console.info(this.uploadFile);
			if(this.uploadFile.length>0&&!isCb){
				webToast("还剩余"+this.uploadFile.length+"张图片没有上传,请稍后","bottom",3000);
			}
			return true;
		},
		// 上传多个文件
		...
		funUploadFile : function(file){
		    ...
		    xhr.addEventListener("load", function(e){
		    	console.log("25");
		    	var result=parseDom(e.currentTarget.response);
		    	var urlParam=result[3].value.split('?').slice(1);
	    		// 从文件中删除上传成功的文件  false是不执行onDelete回调方法
		    	var obj=eval("("+decodeURIComponent(urlParam)+")");
		    	if(obj.ErrorMsg!=null && obj.ErrorMsg!="null"&& obj.ErrorMsg!=""){
		    		console.log("26");
		    		alert(obj.ErrorMsg);
		    ...
		    var newurl=self.url;
			if(window.addExtendParam){
				newurl=addExtendParam(self.url,file);
				if(newurl==""){
					$("#loading").css("display","none");
					return false;
				}
			...
			try{
				...
				xhr.setRequestHeader("X_FILENAME", "unknowimagefile"+postf);
			}
