	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : "http://www.wutuo.help/PictureService/ProdPicture/uploadpic",  	// 上传文件的路径
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size >= 4096000) {
						alert('您这个"'+ file.name +'"文件大小过大,请选择小于4M的图片上传');	
					} else {
						// 在这里需要判断当前所有文件中
						arrFiles.push(file);	
					}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "images/zyUpload/";
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, object) {
						$("#uploadProgress_" + file.index).hide();
						$("#uploadSuccess_" + file.index).show();
						if(object!=null  && object.Status>0){
							var ImageIDs=$("#ImageIDList").val();
							ImageIDs=ImageIDs+","+object.Status;
							$("#ImageIDList").val(ImageIDs);
					...
					onFailure: function(file) {
						$("#uploadProgress_" + file.index).hide();
						$("#uploadSuccess_" + file.index).show();
						$("#uploadInf").text("文件" + file.name + "上传失败！");	
						 $("#loading").css("display","none");
						//$("#uploadImage_" + file.index).css("opacity", 0.2);
					},
					onComplete: function(response){
						console.info(response);
						if(window.submitImage){
							submitImage();
						}
					},
					onDragOver: function() {
