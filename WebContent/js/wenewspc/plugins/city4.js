...
(function($){
	$.fn.citySelect = function(options) {
		var defaults = {
				setId : ['#Provinces' , '#Citys' , '#Areas','#Schools'],                //默认id
				stval : ['请选择省份' , '请选择城市' , '请选择地区','请选择学校'],     //默认值
				czemt : 'i',                        					  //存值元素
		...
		$.fn.liClick = function(){
			...
			$(_setId[0]).on('click','li', function() { //省
			...
			$(_setId[1]).on('click','li', function() { //市
			...
			$(_setId[2]).on('click', 'li', function() { //区
				console.log('a');
				...
				$(_setId[3]).appendlist({
					theindex: '0_' + indA + '_' + indB + '_' + indC
				});
				return indC;

			});
			$(_setId[3]).on('click', 'li', function() { //保险
				console.log('a');
