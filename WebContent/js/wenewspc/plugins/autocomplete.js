				...
				switch(e.which) {
					...
					case 13: // Enter
						if(currentSelection > -1){
							...
							input.val(text.split('】')[1].split('-')[0].split("...")[0]);
						}
						currentSelection = -1;
						proposalList.empty();
			...
			input.bind("change paste keyup", function(e){
				if(e.which != 13 && e.which != 27 
					...
					if(input.val() != ''){
						var word = "^" + input.val() + ".*";
						proposalList.empty();
						$.getJSON($("#basePath").val()+"WeNews/SearchKeyWord",
								{
								query:$("* [name='query']").val()
					                ...
					                if(json && json.Status==true){ 
					                    ...
					                    $.each(json.data,function(index,array){ 
					                    	proposals[index]=array.name+' - '+ array.value2;
					                    });
					                    params.hints=proposals;
					                    for(var test in params.hints){
												...
												var element = $('<li></li>')
													...
													.addClass('proposal')
													.click(function(){
														input.val($(this).html().split('】')[1].split('-')[0].split("...")[0]);
														proposalList.empty();
														params.onSubmit(input.val()+" ");
													})
													.mouseenter(function() {
