...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
 <link rel="shortcut icon" href="http://www.whohelp.cc/WeNewsAgency/favicon.ico"> 
...
<div class="searchAdvertise">
    ...
    <span class="searchAdvertise-span" style="text-align: right;"><span class="searchAdvertise-spant">美丽家乡发言人</span><br>投票排行榜<br>
    <span class="searchAdvertise-spanf">投票周期：2017-07-01 ~ 2017-09-01 <br>(排行榜数据可能会有延时，具体以文章投票数为准) </span></span>
            ...
            <div class="sRankPeopleO_divf ">
               ...
               <c:if  test="${level!='未入榜'}">
                ...
                <c:if test="${notices!=null }">
	                 <span class="rank_name">${notices.title}</span>
	                <span class="rank_school">${Student.name} - ${Student.school}</span>
                </c:if>
                <c:if test="${notices==null }">
...
$(function() {
	...
	function() {
		if (getScrollTop() + getWindowHeight() == getScrollHeight() || $("#_page_").val()==1) {
			...
			$.getJSON("<%=basePath%>Ranking/getphoneWXWDSRanking",
			{
...
function getRankDOM(array){
	var tmp = '<a href="<%=path%>/Product/phoneweChatPordDetail?_type_=2&_pid_='+ array.id +'&_pc_=<%=SmBaseUtil.Random() %>&_area_=&sina=<%=SmBaseUtil.Random() %>">';
	if ((array.level) == 1) {
        tmp +=  '<div class="searchRank" style="">' +
                '<div class="sRankPeopleDbcolor" style="">' +
                '<div class="sRankPeopleO_divf ">' +
                '<img src="'+ array.userImageUrl +'" style="height:51px;">' +
                '<span class="sRankPeopleO_divf_spanone">' + array.level + '</span>' +
                '<span limit="5" class="rank_name"> ' + array.title + '</span>' +
                '<span limit="8" class="rank_school"> ' + array.author + '-'+array.school + '</span>' +
