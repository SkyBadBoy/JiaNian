...
<div class="searchAdvertise">
    <a href="#"><img class="search-imgresp" src="<%=basePath%>images/bg_school.png"></a>
    <!--<span><img src="images/news.png" class="searchAdvertise-img"></span>-->
    <span class="searchAdvertise-span"  style="text-align: right;" >${title}<br>
...
$(function() {
	...
	function() {
		if (getScrollTop() + getWindowHeight() == getScrollHeight() || $("#_page_").val()==1) {
			...
			$.getJSON("<%=basePath%>/Ranking/phonegetNoticesSchoolRankingList",
			{
...
function bindImgError(){
	...
	$("img").error(function () {
	    $(this).unbind("error").attr("src", $("#basePath").val()+"img/schooldefault.png");
	});

}
//获取动态添加的活动和产品信息
function getRankDOM(array){
	var tmp = '<a href="<%=path%>/weChatGroup/phoneWeNewsHome?_isStu_=0&_paid_='+ array.pkid +'&sina=<%=SmBaseUtil.Random() %>">';
	if ((array.level) == 1) {
        tmp +=  '<div class="searchRank" style="">' +
                ...
                '<span limit="8" class="rank_school"> ' + ((array.school==undefined)?"":array.school) + '</span>' +
